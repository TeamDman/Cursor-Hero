...
pub fn wheel_opening(
    mut toolbelt_query: Query<
        (Entity, &ActionState<ToolbeltAction>, &mut Wheel, &Parent, &Children),
        (Without<Tool>, With<Toolbelt>),
    >,
    tool_query: Query<Entity, With<Tool>>,
    mut toolbelt_events: EventWriter<ToolbeltStateEvent>,
) {
    for (toolbelt_id, toolbelt_actions, mut wheel, toolbelt_parent, toolbelt_children) in toolbelt_query.iter_mut() {
        if toolbelt_actions.pressed(ToolbeltAction::Show) {
            ...
            if !wheel.open {
                toolbelt_events.send(ToolbeltStateEvent::Opened { toolbelt_id, character_id: toolbelt_parent.get() });
                wheel.open = true;
            }
        } else {
            if wheel.open {
                toolbelt_events.send(ToolbeltStateEvent::Closed { toolbelt_id, character_id: toolbelt_parent.get() });
                wheel.open = false;
            }
        }
    }
