# {{begin_MemoryPluginBuildConfig}}
#     pub {{crate_name}}_memory_enabled: bool,
# {{middle}}
#             {{crate_name}}_memory_enabled: true,
# {{end}}

def gather_variables(text: str) -> dict[str,str]:
    # begin_MemoryPluginBuildConfig
    find = "pub struct MemoryPluginBuildConfig {"
    include = True
    index = text.find(find)
    assert index != -1, f"Coult not find `{find}`"
    index = index + len(find) if include else index
    begin_MemoryPluginBuildConfig, remaining = text[:index],text[index:]
    ...
    assert index != -1, f"Coult not find `{find}`"
    index = index + len(find) if include else index
    middle, remaining = remaining[:index],remaining[index:]

    # end
    end = remaining

    return {
        "begin_MemoryPluginBuildConfig": begin_MemoryPluginBuildConfig,
        "middle": middle,
        "end": end,
    }
